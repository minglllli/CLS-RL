...
if __name__ == '__main__':
    ...
    OUTPUT_PATH = 'results/'+ ft_method+'_'+args.ft_dataset+'_'+args.prompt+'/'+ ft_method+'_'+args.ft_dataset+'_'+args.prompt+"_Qwen2-VL-2B-Instruct-" + dataset + "-fewshot.json"
    if not os.path.exists('results/'):
        os.makedirs('results/')
    if not os.path.exists('results/'+ ft_method+'_'+args.ft_dataset+'_'+args.prompt):
        # Create the directory
        os.makedirs('results/'+ ft_method+'_'+args.ft_dataset+'_'+args.prompt)
    if os.path.exists(OUTPUT_PATH):
        print(f'{dataset}: file exist')
        sys.exit(0)

    prefix,path_prefix = dataset_prefix(dataset)
    # path to the prompt file, change to your own path
    data_file_path = "/mnt/petrelfs/liming/CLS-RL/src/eval/prompts/datasets_fewshot/" + prefix + "_fewshot_test.jsonl"
    ...
    with open("results/output_map.txt", "a") as f:  # 以写模式 ("w") 打开文件
